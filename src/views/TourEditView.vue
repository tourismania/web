...
function openEdit(type: DialogType, index: number) {
  ...
  if (type === 'flight') draftFlight.value = JSON.parse(JSON.stringify(tour.flights[index]))
  if (type === 'hotel') draftHotel.value = JSON.parse(JSON.stringify(tour.hotels[index]))
  if (type === 'carRental') draftCarRental.value = JSON.parse(JSON.stringify(tour.carRentals[index]))
  if (type === 'cruise') draftCruise.value = JSON.parse(JSON.stringify(tour.cruises[index]))
  if (type === 'excursion') draftExcursion.value = JSON.parse(JSON.stringify(tour.excursions[index]))
  if (type === 'transport') draftTransport.value = JSON.parse(JSON.stringify(tour.transport[index]))
  if (type === 'service') draftService.value = JSON.parse(JSON.stringify(tour.additionalServices[index]))
  activeDialog.value = type
}
...
function saveDialog() {
  ...
  if (activeDialog.value === 'flight') {
    if (idx !== null) tour.flights[idx] = JSON.parse(JSON.stringify(draftFlight.value))
    else tour.flights.push(JSON.parse(JSON.stringify(draftFlight.value)))
  }
  if (activeDialog.value === 'hotel') {
    if (idx !== null) tour.hotels[idx] = JSON.parse(JSON.stringify(draftHotel.value))
    else tour.hotels.push(JSON.parse(JSON.stringify(draftHotel.value)))
  }
  if (activeDialog.value === 'carRental') {
    if (idx !== null) tour.carRentals[idx] = JSON.parse(JSON.stringify(draftCarRental.value))
    else tour.carRentals.push(JSON.parse(JSON.stringify(draftCarRental.value)))
  }
  if (activeDialog.value === 'cruise') {
    if (idx !== null) tour.cruises[idx] = JSON.parse(JSON.stringify(draftCruise.value))
    else tour.cruises.push(JSON.parse(JSON.stringify(draftCruise.value)))
  }
  if (activeDialog.value === 'excursion') {
    if (idx !== null) tour.excursions[idx] = JSON.parse(JSON.stringify(draftExcursion.value))
    else tour.excursions.push(JSON.parse(JSON.stringify(draftExcursion.value)))
  }
  if (activeDialog.value === 'transport') {
    if (idx !== null) tour.transport[idx] = JSON.parse(JSON.stringify(draftTransport.value))
    else tour.transport.push(JSON.parse(JSON.stringify(draftTransport.value)))
  }
  if (activeDialog.value === 'service') {
    if (idx !== null) tour.additionalServices[idx] = JSON.parse(JSON.stringify(draftService.value))
    else tour.additionalServices.push(JSON.parse(JSON.stringify(draftService.value)))
  }
  activeDialog.value = null
}

function removeItem(type: DialogType, index: number) {
  if (type === 'flight') tour.flights.splice(index, 1)
  if (type === 'hotel') tour.hotels.splice(index, 1)
  if (type === 'carRental') tour.carRentals.splice(index, 1)
  if (type === 'cruise') tour.cruises.splice(index, 1)
  if (type === 'excursion') tour.excursions.splice(index, 1)
  if (type === 'transport') tour.transport.splice(index, 1)
  ...
  <v-container class="py-4" max-width="900">
    ...
    <v-card class="mb-3 section-card">
      <v-card-text class="pb-2">
        ...
        <v-row dense>
          <v-col cols="12">
            <v-text-field v-model="tour.title" label="Название тура" density="compact" variant="outlined" hide-details />
          </v-col>
          <v-col cols="6">
            <v-text-field v-model="tour.startDate" label="Дата начала" type="date" density="compact" variant="outlined" hide-details />
          </v-col>
          <v-col cols="6">
            <v-text-field v-model="tour.endDate" label="Дата окончания" type="date" density="compact" variant="outlined" hide-details />
          </v-col>
          <v-col cols="12">
            <v-textarea v-model="tour.welcomeText" label="Приветственный текст" density="compact" variant="outlined" rows="3" hide-details auto-grow />
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.flights.length" size="x-small" color="primary" class="ml-1">{{ tour.flights.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(f, i) in tour.flights" :key="i" class="entity-row">
          ...
          <div class="d-flex align-center ga-3 mt-2">
            ...
            <div class="d-flex align-center ga-2">
              <v-text-field v-model.number="tour.totalFlightsCost" label="Итого" density="compact" variant="outlined" type="number" hide-details style="width:130px" />
              <v-select v-model="tour.flightsCurrency" :items="CURRENCIES" label="Валюта" density="compact" variant="outlined" hide-details style="width:100px" />
            </div>
          </div>
        </v-expansion-panel-text>
      </v-expansion-panel>
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.hotels.length" size="x-small" color="primary" class="ml-1">{{ tour.hotels.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(h, i) in tour.hotels" :key="i" class="entity-row">
            <div class="entity-info">
          ...
          <div class="d-flex align-center ga-3 mt-2">
            ...
            <div class="d-flex align-center ga-2">
              <v-text-field v-model.number="tour.totalHotelsCost" label="Итого" density="compact" variant="outlined" type="number" hide-details style="width:130px" />
              <v-select v-model="tour.hotelsCurrency" :items="CURRENCIES" label="Валюта" density="compact" variant="outlined" hide-details style="width:100px" />
            </div>
          </div>
        </v-expansion-panel-text>
      </v-expansion-panel>
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.carRentals.length" size="x-small" color="primary" class="ml-1">{{ tour.carRentals.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(c, i) in tour.carRentals" :key="i" class="entity-row">
            <div class="entity-info">
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.cruises.length" size="x-small" color="primary" class="ml-1">{{ tour.cruises.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(c, i) in tour.cruises" :key="i" class="entity-row">
            <div class="entity-info">
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.excursions.length" size="x-small" color="primary" class="ml-1">{{ tour.excursions.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(e, i) in tour.excursions" :key="i" class="entity-row">
            <div class="entity-info">
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.transport.length" size="x-small" color="primary" class="ml-1">{{ tour.transport.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(t, i) in tour.transport" :key="i" class="entity-row">
            <div class="entity-info">
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-2">
            ...
            <v-chip v-if="tour.additionalServices.length" size="x-small" color="primary" class="ml-1">{{ tour.additionalServices.length }}</v-chip>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-for="(s, i) in tour.additionalServices" :key="i" class="entity-row">
            <div class="entity-info">
