...
import type {Client, Tour} from '@/api/types/tour'
...
export const useTourStore = defineStore('tour', {
  ...
  getters: {
    tourById: (state) => (id: string) => state.tours.find((t) => t.id === id) ?? null,
    toursCount: (state) => state.tours.length,
  },
  ...
  actions: {
    async loadTours() {
      ...
      this.error = null
      try {
        this.tours = await TourApi.getAll()
      } catch {
    ...
    async loadTourById(id: string) {
      this.loading = true
      this.error = null
      try {
        this.currentTour = await TourApi.getById(id)
      } catch {
        this.currentTour = MOCK_TOURS.find((t) => t.id === id) ?? null
        if (!this.currentTour) {
    ...
    async createTour(data: Partial<Tour>): Promise<Tour | null> {
      this.loading = true
      this.error = null
      try {
        const created = await TourApi.create(data)
        this.tours.push(created)
        return created
      } catch {
        this.error = 'Не удалось создать тур'
        return null
      } finally {
        this.loading = false
      }
    ...
    async updateTour(id: string, data: Partial<Tour>): Promise<Tour | null> {
      ...
      this.error = null
      try {
        const updated = await TourApi.update(id, data)
        const idx = this.tours.findIndex((t) => t.id === id)
        if (idx !== -1) this.tours[idx] = updated
        if (this.currentTour?.id === id) this.currentTour = updated
        return updated
      } catch {
        this.error = 'Не удалось обновить тур'
        return null
      } finally {
        this.loading = false
      }
    ...
    async deleteTour(id: string): Promise<boolean> {
      ...
      try {
        ...
        if (this.currentTour?.id === id) this.currentTour = null
        return true
      } catch {
        this.error = 'Не удалось удалить тур'
        return false
      } finally {
        this.loading = false
      }
