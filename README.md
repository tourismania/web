...
|---|---|---|
| `city` | `string` | Город |
| `dateTime` | `string` | Локальное время аэропорта (ISO без TZ-суффикса, напр. `2026-05-14T10:00:00`) |
| `timezone` | `string` | IANA-зона аэропорта (напр. `Europe/Moscow`, `Asia/Tokyo`). Нужна для корректного расчёта длительности перелёта между разными зонами |
